...
public class PhotoIdFragment extends Fragment {
    ...
    TextView noData;
    String path = (Environment.getExternalStorageDirectory().toString() + "/Document Scanner/Photo ID");
    int pos;
    ...
    public View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        ViewGroup viewGroup2 = (ViewGroup) layoutInflater.inflate(R.layout.fragment_photo_id, viewGroup, false);

        this.rvCreation = (RecyclerView) viewGroup2.findViewById(R.id.rvPhotoIdCreation);
        ...
        ItemClickSupport.addTo(this.rvCreation).setOnItemClickListener(new ItemClickSupport.OnItemClickListener() {
            public void onItemClicked(RecyclerView recyclerView, int i, View view) {
                PhotoIdFragment.this.pos = i;
                if (PhotoIdFragment.this == null) {
                    Intent intent = new Intent(PhotoIdFragment.this.getActivity(), Camera_Scanner_CreationListActivity.class);
                    intent.putExtra("folderPath", PhotoIdFragment.this.creationParentModels.get(i).getFolderPath());
                    PhotoIdFragment.this.startActivity(intent);
                } else {
                    Intent intent2 = new Intent(PhotoIdFragment.this.getActivity(), Camera_Scanner_CreationListActivity.class);
                    intent2.putExtra("folderPath", PhotoIdFragment.this.creationParentModels.get(i).getFolderPath());
                    PhotoIdFragment.this.startActivity(intent2);
                }
            }
        });
        return viewGroup2;
