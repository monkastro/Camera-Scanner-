...
public class TextFilesFragment extends Fragment {
    ...
    TextView noData;
    String path = (Environment.getExternalStorageDirectory().toString() + "/Document Scanner/OCR Files");
    int pos;
    ...
    public View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        ViewGroup viewGroup2 = (ViewGroup) layoutInflater.inflate(R.layout.fragment_text_files, viewGroup, false);

        this.rvCreation = (RecyclerView) viewGroup2.findViewById(R.id.rvOCRCreation);
        ...
        ItemClickSupport.addTo(this.rvCreation).setOnItemClickListener(new ItemClickSupport.OnItemClickListener() {
            public void onItemClicked(RecyclerView recyclerView, int i, View view) {
                TextFilesFragment.this.pos = i;
                if (TextFilesFragment.this == null) {
                    Intent intent = new Intent(TextFilesFragment.this.getActivity(), Camera_Scanner_CreationListActivity.class);
                    intent.putExtra("folderPath", TextFilesFragment.this.creationParentModels.get(i).getFolderPath());
                    TextFilesFragment.this.startActivity(intent);
                }  else {
                    Intent intent2 = new Intent(TextFilesFragment.this.getActivity(), Camera_Scanner_CreationListActivity.class);
                    intent2.putExtra("folderPath", TextFilesFragment.this.creationParentModels.get(i).getFolderPath());
                    TextFilesFragment.this.startActivity(intent2);
                }
            }
        });
        return viewGroup2;
