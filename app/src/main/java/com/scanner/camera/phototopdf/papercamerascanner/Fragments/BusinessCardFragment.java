...
public class BusinessCardFragment extends Fragment {
    ...
    TextView noData;
    String path = (Environment.getExternalStorageDirectory().toString() + "/Document Scanner/Business Card");
    int pos;
    RecyclerView rvCreation;


    public View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        ViewGroup viewGroup2 = (ViewGroup) layoutInflater.inflate(R.layout.fragment_business_card, viewGroup, false);

        this.rvCreation = (RecyclerView) viewGroup2.findViewById(R.id.rvBusinessCreation);
       ...
       ItemClickSupport.addTo(this.rvCreation).setOnItemClickListener(new ItemClickSupport.OnItemClickListener() {
            public void onItemClicked(RecyclerView recyclerView, int i, View view) {
                BusinessCardFragment.this.pos = i;
                if (BusinessCardFragment.this == null) {
                    Intent intent = new Intent(BusinessCardFragment.this.getActivity(), Camera_Scanner_CreationListActivity.class);
                    intent.putExtra("folderPath", BusinessCardFragment.this.creationParentModels.get(i).getFolderPath());
                    BusinessCardFragment.this.startActivity(intent);
                } else {
                    Intent intent2 = new Intent(BusinessCardFragment.this.getActivity(), Camera_Scanner_CreationListActivity.class);
                    intent2.putExtra("folderPath", BusinessCardFragment.this.creationParentModels.get(i).getFolderPath());
